...
Ten plik żyje w repo `bank-contract` i trafia do każdego repo aplikacji jako submoduł `contract/`. Codex wczytuje
go przez `AGENTS.md` danego repo. Zawiera wyłącznie zasady, które się nie zmieniają, oraz wskazówki, gdzie szukać
...
| Kiedy | Czytaj |
|---|---|
| Zawsze | ten plik, potem `AGENTS.md` w root repo aplikacji, nad którą pracujesz |
