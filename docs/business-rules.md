...
- EOD (`POST /business-days:close` lub scheduler 23:00 Europe/Warsaw) działa jako uporządkowany łańcuch zadań; każdy krok jest idempotentny i logowany w `job_run`:
  1. `OPEN → CLOSING` (nowe księgowania odrzucane z `business-day-closed`).
  ...
  6. Snapshoty sald dla wszystkich kont GL; uzgodnienie: Σ snapshotów wg typu musi spełniać niezmiennik GL, `2300` i `9000` muszą być zerowe, Σ `2000-*` = `2000`. Niepowodzenie → zadanie `FAILED`, dzień zostaje `CLOSING`, alert. Nigdy nie naprawiaj automatycznie.
...
- Rachunek zamykany w środku okresu → kapitalizacja przy zamknięciu tą samą regułą, zanim dopuścimy `CLOSED`.

## 9. Storna i korekty

- Operatorzy mogą wystornować dowolny zapis (ADMIN) — powstaje `REVERSAL` wskazujący na oryginał; oryginał pozostaje nietknięty.
- Zapisy ręczne (ADMIN) muszą się bilansować, nie mogą dotykać `2000-*` bez udokumentowanego powodu i są oflagowane w audycie.
- Bez datowania wstecz: `business_date` każdego nowego zapisu równa się bieżącemu dniu `OPEN`, z wyjątkiem zadań EOD, które księgują w dzień `CLOSING`.
